...
void flip_test(std::atomic<bool> &atomic_bool) { atomic_bool.exchange(!atomic_bool.load()); }

void add_test(std::atomic<int> &a, std::atomic<int> &b, std::atomic<int> &result) { result = (a.load() + b.load()); }

void slp_test(int num_milliseconds) { std::this_thread::sleep_for(std::chrono::milliseconds(num_milliseconds)); }
...
void test_NTasksMThreads_sleep(int sleep_time, int num_tasks, int num_threads) {
    ...
    // accounting
    int parallel_time =
        sleep_time * (int)std::ceil((double)num_tasks / (double)num_threads) + (num_tasks * num_threads);
    ...
    auto parallel_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(parallel_duration).count();
    ...
    auto linear_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(linear_duration).count();
...
TEST_F(ThreadPoolTest, ZeroThreads) { EXPECT_THROW(ThreadPool(0), std::out_of_range); }
...
TEST_F(ThreadPoolTest, MultipleThreadSingleTask) {
    ...
    small_thread_pool_->add_task(add_test, std::ref(atomic_int_1), std::ref(atomic_int_2), std::ref(atomic_int_result))
        .get();
...
TEST_F(ThreadPoolTest, ExceptContinue) {
    ...
    int expected_time_max = sleep_time_mult * sleep_time + (sleep_time_mult * SMALL_POOL_SIZE);
