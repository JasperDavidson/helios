...
int main() {
    ...
    DataHandle<std::vector<float>> vec1_handle = data_manager.create_data_handle(vec1);
    DataHandle<std::vector<float>> vec2_handle = data_manager.create_data_handle(vec2);
    DataHandle<int> dot_product_out = data_manager.create_ref_handle(&dot_result);

    std::vector<DataUsage> data_usages = {DataUsage::ReadOnly, DataUsage::ReadOnly, DataUsage::ReadWrite};

    auto dot_product_task = CPUTask("dot_product", std::vector(vec1_handle.id, vec2_handle.id), dot_product_out.id,
                                    data_manager, data_usages, dot_product, vec1_handle, vec2_handle);

    TaskGraph task_graph;
    task_graph.add_task(std::make_shared<decltype(dot_product_task)>(dot_product_task));

    Runtime helios_runtime(data_manager, 4);
    GPUDevice gpu_device(GPUBackend::Metal);
